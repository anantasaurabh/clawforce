...
export default function Users() {
  ...
  const handleResetPassword = async (email, name) => {
    if (window.confirm(`System Protocol: Are you sure you want to trigger a password reset for ${name}? This will send an encrypted recovery link to ${email}.`)) {
      try {
        ...
        alert(`Reset Protocol Initiated: A recovery transmission has been dispatched to ${email}.`);
      } catch (err) {
        console.error(err);
  ...
  const handleDelete = async (uid, name) => {
    if (window.confirm(`Critical Protocol: Are you sure you want to decommission personnel "${name}"? This will terminate their clearance and remove their profile from the active registry.`)) {
      try {
        setLoading(true);
        ...
        console.error(err);
        alert('Critical Error: Failed to decommission personnel.');
      } finally {
        setLoading(false);
      }
    }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50">
                ...
                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Assigned Package</th>
                <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Last Seen</th>
  ...
  const handleResetPassword = async () => {
    if (window.confirm(`System Protocol: Are you sure you want to trigger a password reset for ${formData.displayName}?`)) {
      try {
        ...
        alert(`Reset Protocol Initiated: A recovery transmission has been dispatched to ${formData.email}.`);
      } catch (err) {
        console.error(err);
        alert('Critical Error: Failed to initiate reset protocol.');
      } finally {
        ...
        <form onSubmit={handleSubmit} className="p-8 pt-4 space-y-10 max-h-[70vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black text-slate-800 tracking-tight">User Information</h2>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black text-slate-800 tracking-tight">Login Credentials</h2>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black text-slate-800 tracking-tight">Access & Status</h2>
            </div>
