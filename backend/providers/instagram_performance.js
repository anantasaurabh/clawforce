...
export async function fetchMetrics(timeRange, targetId) {
  try {
    ...
    const base = allData.instagram || { summary: { reach: 0, engagement: 0, conversion: 0 }, chartData: [] };

    if (!targetId) return base;

    // Deterministic scaling based on targetId string
    const hash = targetId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const factor = 0.5 + (hash % 10) / 5;
